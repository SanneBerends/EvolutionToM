...
def plot_exp_lengths():
    ...
    # labels/titles
    plt.title("EXP1: Distribution of Experiment Lengths, \n $\\lambda \\in$ [0.2, 0.5, 0.8]")
    plt.xlabel("Lambda")
...
def plot_distrs():
    ...
    for condition in [("EXP1: Average Final Distribution (one type left), \n $\\lambda$ $\\in$ [0.2, 0.5, 0.8]",
        ...
        plt.grid(axis='y', color='lightgray', linestyle='--', linewidth=0.5, alpha=0.7)
        plt.tight_layout()
        if title == "EXP1: Average Final Distribution (one type left), \n $\\lambda$ $\\in$ [0.2, 0.5, 0.8]":
...
def plot_dom_freqs():
    ...
    ax.set_title('EXP1: Average Dominance Frequencies for ToM0, ToM1, and ToM2, \n $\\lambda$ $\\in$ [0.2, 0.5, 0.8]')
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend()
    ...
    plt.grid(axis='y', color='lightgray', linestyle='--', linewidth=0.5, alpha=0.7)
    plt.tight_layout()
    plt.savefig('avg_dom_freqs.png', dpi=300)
    plt.show()
...
def plot_zoomed_dom_freqs():
    ...
    ax.set_xlabel('ToM orders')
    ax.set_ylabel('Average Dominance Frequency')
    ax.set_title('EXP1: Average Dominance Frequencies for ToM1 and ToM2, \n $\\lambda$ $\\in$ [0.2, 0.5, 0.8]')
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend()
...
def plot_ages():
    ...
    for l_value in [2,5,8]:
        ...
        # labels/titles
        plt.title(f"EXP1: Distribution of Ages per ToM Order, \n $\\lambda$ = {l_value/10}")
...
def plot_neg_lengths():
    ...
    ax.set_title('EXP1: Average Negotiation Length,\n $\\lambda$ $\\in$ [0.2, 0.5, 0.8]')
    ax.set_xticks(x)
...
def plot_neg_endings():
    ...
    for i in [2, 5, 8]:
        ...
        ax.set_title(f"EXP1: Relative Frequency of Negotiation Endings by Type, \n $\\lambda$ = {l_value / 10}")
...
def plot_pi_gains():
    ...
    for i in [2, 5, 8]:
        ...
        plt.title(f'EXP1: Average Gains of Initiator vs Other by Negotiation Type, \n $ \\lambda $ = {l_value / 10}')
...
def main():
    ...
    folder_name_1 = "exp1_l0.2"
    folder_name_2 = "exp1_l0.5"
    folder_name_3 = "exp1_l0.8"

    for i in range(0, n_exps):
        filename = "exp1_L2_" + str(i) + "_0.csv"
        filepath = os.path.join(folder_name_1, filename)
        loop_files(filepath, 2)

        filename = "exp1_L5_" + str(i) + "_0.csv"
        filepath = os.path.join(folder_name_2, filename)
        loop_files(filepath, 5)

        filename = "exp1_L8_" + str(i) + "_0.csv"
